...
$newCustomerMachine = new CustomerMachine($dbLink, $_SESSION['machine_type']);
...
<script language="javascript" >
	function checkRequiredFields(){
	  ...
	  || document.getElementById('plc_ip').value == "" ){
	    ...
	    return false;
	  }else if(document.getElementById('hmi_ip').value.length < 9 || document.getElementById('hmi_ip').value.length > 13) {
...
<div class='pageTitle' >Customer HC</div>
...
<?php if($appManager->isAdmin()) {?>
	<div class="formBackground_adm">
		<form name="addCustomerFusionForm" action="add_customer_hc.php" method="POST" onsubmit="return checkRequiredFields()" >
		Select a Customer 
		<select id="customer_id" name="customer_id">
		<?php echo $newCustomer->getSelectCustomerOptions('h'); ?>
		</select></br>
		Select a HC HMI
		...
		<input type="submit" value="Add Customer HC" /> 
		</form> 
	</div>
<?php } ?>
